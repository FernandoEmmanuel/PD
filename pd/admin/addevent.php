...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (isset($_POST['add'])) {
    $stmt = $con->prepare("INSERT INTO `events` (id, name, venue, date, time) VALUES (?, ?, ?, ?, ?)");
  ...
  if (isset($_POST['update'])) {
    $stmt = $con->prepare("UPDATE `events` SET name = ?, venue = ?, date = ?, time = ? WHERE id = ?");
  ...
  if (isset($_POST['delete'])) {
    ...
    if ($stmt->execute()) {
      echo '<script>alert("Upcoming event deleted successfully.");</script>';
    } else {
      echo "Error: " . $stmt->error;
    }
    $stmt->close();
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">
              <?php echo isset($_SESSION['fllname']) ? ucwords($_SESSION['fllname']) : 'User'; ?>'s Profile | Add Fixture
            </h1>
          </div>
